...
X_ro = np.stack(map(read_pgm2, dataset0.data))

# Copy the features to create a mutable feature matrix
X_cp = np.copy(X_ro)

def random_occlusion(X):
    N = len(X)
    rnd_occ = sorted(random.sample(xrange(N), 500))
    #print rnd_occ
    for i in rnd_occ:
        X[i][10:90,10:-10] = 0
...
print
print 
print "Occluded Images Genearated ........"
print
print
print
print
...
print "Combining A and E matrix horizontaly"
...
pca = PCA(n_components=300)
pc_samples = pca.fit_transform(C)
